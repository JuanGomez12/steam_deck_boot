...
def test_new_movie_renaming(tmp_path: Path):
    ...
    assert movie_randomizer.startup_movie_path.is_file() == True
    assert new_startup_movie_path.is_file() == True


def test_error_wrong_movies_path_type():
    with pytest.raises(TypeError, match=r"Expected Path type, received .*"):
        movie_randomizer = MovieRandomizer("test")


def test_error_missing_movies_path(tmp_path):
    with pytest.raises(FileNotFoundError, match=r".* is not a directory/does not exist"):
        movie_randomizer = MovieRandomizer(tmp_path /"test")
